<template>
  <div style="padding: 30px">
    ...
    <div class="search-bar">
      ...
      <el-select v-model="filterData.disabled" size="mini" clearable placeholder="请选择是否停用">
        <el-option
          label="是"
          value="1"
        />
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            type="text"
            size="small"
            @click.native.prevent="banpickHanldle(scope.row,scope.$index)"
          >
            {{scope.row.disabledC===0?'停用':'启用'}}
          </el-button>
          <el-button
            type="text"
            size="small"
    ...
    <el-dialog :close-on-click-modal="false" width="80%" :visible.sync="dialogIntegralDetail" title="查看用户积分预收余额">
      ...
      <select-table :data="integraDetailList" :pagination-data="paginationData2"
                    @paginationChange="checkIntegralDetail"/>
    </el-dialog>
      ...
      banpickHanldle(row, index) {
        ...
        if (row.disabledC == 0) {
          this.$prompt('请输入备注', '此操作将停用客户', {
            ...
            putClientsDisabled(params, this.userId).then(res => {
