<template>
  <div style="padding: 10px">
    ...
    <div id="printContainerC">
      <div
        v-for="(item1,index1) in splitData"
        :key="index1"
        class="printContainer"
        :style="{'marginBottom':paperBottom+'px'}"
      >
        <p class="order-title">
          {{ storeName }}-{{ orderName }}
        </p>
        <p class="head first">
          <span v-if="orderId">
            单号：{{ orderId }}
          </span><span>制单日期：{{ orderTime }}</span><span>第{{ index1+1 }}/{{ splitData.length }}页</span>
        </p>
        <p v-if="supplierName" class="head">
          <span>供应商：{{ supplierName }}</span><span>电话:{{ supplierPhone }}</span><span :style="{'paddingRight':supplierAddr?'0':'100px'}">
            地址：{{ supplierAddr }}
          </span>
        </p>
        <p v-if="clientName" class="head">
          <span>客户：{{ clientName }}</span><span>电话:{{ clientPhone }}</span><span :style="{'paddingRight':clientAddr?'0':'100px'}">
            地址：{{ clientAddr }}
          </span>
        </p>
        <div class="select-table">
          ...
          <div style="font-size: 12px;display: flex;width: 100%;border: 1px solid black;border-top: 0;">
            <span
              style="box-sizing:border-box;padding: 3px 0;width:449px;text-align: right;padding-right: 4px;"
            >
              本页小计：
            </span>
            <span
              style="box-sizing:border-box;padding: 3px 0;width: 116px;text-align: center;border-left: 1px solid black;"
            >
              {{ splitTotalArr[index1].quantityC }}
            </span>
            <span
              style="box-sizing:border-box;padding: 3px 0;width: 117px;text-align: center;border-left: 1px solid black;border-right: 1px solid black;"
            >
              {{ splitTotalArr[index1].moneyC }}
            </span>
          </div>
          <div style="font-size: 12px;display: flex;width: 100%;border: 1px solid black;border-top: 0;">
            <span
              style="box-sizing:border-box;padding: 3px 0;width: 180px;text-align: right;padding-right: 4px;border-right: 1px solid;"
            >
              合计金额（大写）：
            </span>
            <span style="box-sizing:border-box;padding: 3px 0;width: 269px;text-align: left;padding-right: 4px;padding-left: 4px">
              {{ NumberToChinese(totalMoney.toFixed(2)) }}
            </span>
            <span style="box-sizing:border-box;padding: 3px 0;flex: 1;text-align: center;border-left: 1px solid black;">
              {{ '¥：'+totalMoney.toFixed(2)+'元' }}
            </span>
          </div>
        </div>
        <p class="footer">
          <span>制单人：{{ name }}</span><span v-if="settlementType">
            结算方式:{{ settlementType }}
          </span>
        </p>
        <p class="footer">
