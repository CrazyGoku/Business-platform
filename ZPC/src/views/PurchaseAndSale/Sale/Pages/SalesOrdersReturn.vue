  ...
  <div>
    <div class="handle-bar">
      <el-button
        ...
        size="mini"
        @click="directDialog = true;isEdit = false;chioceSelect={};choiceGoodsSku = [];createTime = ''"
      >
        直接添加
      </el-button>
      <el-button
    ...
    <el-dialog :close-on-click-modal="false" width="80%" :visible.sync="addVisible" :title="isEdit?'编辑订单':'添加退货订单'">
        ...
        >
          <template slot-scope="scope">
            <el-button
              type="text"
              size="small"
              @click.native.prevent="addBtnFun(scope.$index,scope.row,false);createTime = ''"
            >
              退货
            </el-button>
          </template>
...
export default {
  ...
  methods: {
    ...
    comfirm() {
      ...
      data.createTime = this.isEdit ? '' : this.createTime
      func(data).then(res => {
        if (res.data.code !== 1001) {
          this.$message({
            showClose: true,
