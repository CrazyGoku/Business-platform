<template>
  <div>
    ...
    <el-dialog width="80%" :close-on-click-modal="false" :visible.sync="addVisible" :title="isEdit?'编辑订单':'添加订单'">
      ...
      >
        ...
        <el-table-column prop="totalMoney" label="总价" width="180" align="center">
          <template scope="scope">
            {{ (scope.row.money*scope.row.quantity).toFixed(2) }}
          </template>
        </el-table-column>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="printDialog = false;payType = []">
          取 消
        </el-button>
    ...
    comfirm() {
      ...
      this.choiceGoodsSku.forEach(v => {
        ...
        if (v.isGift) {
          ...
          outTotalQuantity += Number(v.quantity)
          totalMoney += Number(v.money * v.quantity.toFixed(2))
          _detail = {
            'type': 0,
            'goodsSkuId': v.id,
            'quantity': v.quantity,
