<template>
  <div>
    ...
    <div class="flex-center">
      ...
      >
        ...
        <el-table-column label="赠品" width="180" align="center">
          <template scope="scope">
            <el-checkbox v-model="scope.row.isGift" />
          </template>
        </el-table-column>
        <el-table-column
          prop="goodsName"
        ...
        <el-card class="box-card">
          ...
          <el-row :gutter="20">
            <el-col :span="12" class="icon-box">
              <el-checkbox-group v-model="checkList" :min="1" @change="changeCheck">
                <el-checkbox label="1">
                  <div class="icon-item">
              ...
              <el-checkbox v-model="groupCheck" @change="groupChange">
                组合收款
              </el-checkbox>
            </el-col>
            <el-col :span="12" class="input-con">
              ...
              <p>
                应收金额: <span style="color: orangered;">
                  {{ receivable }}
                </span>
              </p>
              <p>
                预收余额: <span style="color: orangered;">
                  {{ fundAdvance }}
                </span>
              </p>
              <NumberInput
    ...
    <div v-if="choiceGoodsSku.length>0" style="margin-top: 10px;padding-bottom: 10px;">
      <el-button style="float: right;margin-right: 10px;" type="primary" @click="addFun">
        收银
      </el-button>
    </div>
...
import { getBankAccount, getGoodSkuPrice } from '@/service/PurchaseAndSale/common.js'

export default {
  name: 'RetailOrder',
  components: { SelectTable },
  mixins: [common, warehouseList, clientsList],
  data() {
    return {
      totalInfo: {
        goodsTotal: 0,
      ...
      createTime: '',
      bankAccountList: [],
      followPriceStatus: window.localStorage.getItem('fallowPriceStatus') === 'true'
    }
  },
  computed: {},
  watch: {
    ...
    choiceClient: {
      deep: true,
      handler() {
        if (this.followPriceStatus && JSON.stringify(this.choiceClient) !== '{}') {
          this.choiceGoodsSku.forEach(v => {
            const params = {
              storeId: this.storeId,
              clientId: this.choiceClient.id,
              goodsSkuId: v.goodsSkuId
            }
            getGoodSkuPrice(params).then(res => {
              let fallowPrice = 0
              if (res.data.code == '1001' && res.data.data) {
                fallowPrice = res.data.data.money ? res.data.data.money : 0
              } else {
                fallowPrice = 0
              }
              v.money = fallowPrice || v[this.priceTypeMap[this.choiceClient.clientLevel.priceType]]
            })
          })
        }
      }
  ...
  methods: {
    ...
    watchChange() {
      ...
      if (this.addDetails.discountCouponId) {
        ...
        this.receivable = (totalMoney - this.discountMoney - this.couponAmount).toFixed(2)
      } else {
        this.couponAmount = 0
        this.receivable = (totalMoney - this.discountMoney).toFixed(2)
      }

      this.totalInfo.vipPrice = this.totalInfo.vipPrice.toFixed(2)
      this.totalInfo.goodsTotal = this.totalInfo.goodsTotal.toFixed(2)
    ...
    choiceGoodsSkuFun(value) {
      ...
      if (this.followPriceStatus && JSON.stringify(this.choiceClient) !== '{}') {
        ...
        getGoodSkuPrice(params).then(res => {
          ...
          const money = fallowPrice || _data[this.priceTypeMap[this.choiceClient.clientLevel.priceType]]
          const discountRate = this.choiceClient.clientLevel.price
          _data.quantity = 1
          _data.type = 1
        ...
        if (JSON.stringify(this.choiceClient) !== '{}') {
          money = _data[this.priceTypeMap[this.choiceClient.clientLevel.priceType]]
          discountRate = this.choiceClient.clientLevel.price
        } else {
          money = _data.retailPrice
          discountRate = 1
        }
        _data.quantity = 1
        _data.type = 1
    ...
    addFun() {
      ...
      this.choiceGoodsSku.forEach((v, index) => {
        ...
        if (v.isGift) {
          ...
          _detail.remark = '赠品'
          if (index === num - 1) {
            _detail.discountMoney = mon + mod
          } else {
            _detail.discountMoney = mon
          }
        } else {
          if (index === num - 1) {
          ...
          data.outTotalQuantity += Number(v.quantity)
          data.totalMoney += Number(v.money * v.discountRate * v.quantity)
          _detail.type = 0
          _detail.goodsSkuId = v.goodsSkuId
          _detail.quantity = v.quantity
      ...
      data.orderMoney = (data.totalMoney - data.totalDiscountMoney).toFixed(2)
