        ...
        >
          <template slot-scope="scope">
            <el-button
              type="text"
              size="small"
              @click.native.prevent="getHandle(scope.$index,scope.row);createTime = ''"
            >
              收货
            </el-button>
            <el-button
...
export default {
  ...
  methods: {
    ...
    getHandle(index, row) {
      ...
      func(params, path).then(res => {
        ...
        for (let i = data.details.length - 1; i >= 0; i--) {
          ...
          if (data.details[i].notFinishQuantity == 0 || data.details[i].type == 0) {
    ...
    confirmHandle() {
      ...
      const _detailType = '1'
      let isSell = false
      if (this.orderDetail.typeName === '销售退货申请单' || this.orderDetail.typeName === '销售换货申请单') {
        isSell = true
