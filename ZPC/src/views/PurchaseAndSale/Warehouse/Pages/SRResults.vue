<template>
  <div>
    ...
    <div class="search-bar">
      ...
      <el-select
        v-model="chioceSelect.warehouseId"
        size="mini"
        filterable
        placeholder="请选择仓库"
      >
    ...
    <div class="flex-center">
      <select-table
        ...
        @paginationChange="getStorageFun"
      >
        <el-table-column
          slot="handle"
          :fixed="true"
        ...
        >
          <template slot-scope="scope">
            <el-button
              type="text"
              size="small"
              @click.native.prevent="postRedDashedFun(scope.$index,scope.row)"
            >
              红冲
            </el-button>
            <!--<el-button
    ...
    <el-dialog :close-on-click-modal="false" :visible.sync="orderVisible" title="订单详情">
      <el-table :data="orderDetails">
        ...
        />
        <el-table-column
          v-for="(item,index) in otherEntryMap"
          :key="index"
...
import { otherEntryMap, orderType } from '@/views/PurchaseAndSale/Warehouse/config.js'
...
export default {
  ...
  methods: {
    ...
    getStorageFun() {
      ...
      getStorage(params, this.flag).then(res => {
    ...
    readRow(index, row) {
      ...
      const path = row.id
      getStorageById(params, path).then(res => {
        const data = res.data.data
