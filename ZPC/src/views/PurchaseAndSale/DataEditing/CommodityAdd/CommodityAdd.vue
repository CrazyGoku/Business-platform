  ...
  <div>
    ...
    <div class="handle-bar">
      ...
      <el-button
        ...
        icon="el-icon-circle-plus-outline"
        size="mini"
        @click="downloadFile('/goods/import/template',{storeId:storeId,token:getToken()})"
      >
        导出模版
      </el-button>
    ...
    >
      ...
      <div class="dialog-content-input">
        ...
        <el-select
          v-model="commodityDetail.special"
          size="mini"
          placeholder="请选择是否专区商品"
        >
          <el-option
            label="否"
            :value="0"
      ...
      >
        ...
        <el-table-column label="进价" width="180" align="center">
          <template scope="scope">
            <NumberInput v-model="scope.row.purchasePrice" :no-slot="false" />
          </template>
        </el-table-column>
        <el-table-column label="零售价" width="180" align="center">
          <template scope="scope">
            <NumberInput v-model="scope.row.retailPrice" :no-slot="false" />
          </template>
        </el-table-column>
        <el-table-column label="VIP售价" width="180" align="center">
          <template scope="scope">
            <NumberInput v-model="scope.row.vipPrice" :no-slot="false" />
          </template>
        </el-table-column>
        <el-table-column label="店长价" width="180" align="center">
          <template scope="scope">
            <NumberInput v-model="scope.row.bossPrice" :no-slot="false" />
          </template>
        </el-table-column>
        <el-table-column label="固定提成金额" width="180" align="center">
          <template scope="scope">
            <NumberInput v-model="scope.row.pushMoney" :no-slot="false" />
          </template>
        </el-table-column>
      ...
      <quillEditor
        ref="myQuillEditor"
        v-model="commodityDetail.details"
        class="eu"
...
export default {
  ...
  methods: {
    onEditorChange({ editor, html, text }) { // 内容改变事件
      console.log('---内容改变事件---')
      this.commodityDetail.content = html
      console.log(html)
    },
    beforeUpload() {
      // 显示loading动画
    ...
    uploadSuccess(res, file) {
      ...
      if (res.code === 1001) {
        ...
        quill.insertEmbed(length, 'image', this.BASE_URL + '/main_store' + res.data)
        // 调整光标到最后
    ...
    confirmHandle() {
      ...
      _specifucatuibsList.forEach(v => {
        ...
        if (v.pushMoney == 0 || v.pushMoney == '') {
          v.pushMoney = 0
        }
      })
      var hash = {}
      _allSku.reduce(function(item, next) {
        if (!hash[next.key]) hash[next.key] = []
        hash[next.key].push(next.value)
      }, {})
      for (const i in hash) {
        hash[i] = [...new Set(hash[i])]
      }
      data.skus = JSON.stringify(hash)
      ...
      if (!this.isEdit) {
        ...
        putCommodityData(data).then(res => {
          if (res.data.code !== 1001) {
            this.$message({
              showClose: true,
              message: res.data.message + res.data.details,
              type: 'error'
            })
            return
          }
