  ...
  <div>
    <div class="handle-bar">
      ...
      <el-button
        ...
        icon="el-icon-circle-plus-outline"
        size="mini"
        @click="downloadFile('/clients/import/template',{token:getToken()})"
      >
        导出模版
      </el-button>
    </div>
      ...
      >
        <template slot-scope="scope">
          ...
          <el-button
            type="text"
            size="small"
            @click.native.prevent="changeIntegral(scope.row,scope.$index)"
          >
            修改积分
          </el-button>
        </template>
    ...
    <el-dialog :close-on-click-modal="false" :visible.sync="dialogIntegralVisible" title="查看用户积分提成">
      <!--<select-table :data="integraDetail" :pagination-data="paginationData2" />-->
      <p>积分:{{ integraDetail.integral }}</p>
      <p>提成:{{ integraDetail.pushMoney }}</p>
    </el-dialog>
    ...
    >
      <div style="margin-bottom: 10px" class="search-bar">
        <el-select v-model="exportFilter.levelId" size="mini" placeholder="请选择客户级别">
          <el-option
            v-for="item in levelList"
            :key="item.id"
            :label="item.name"
        ...
        <el-input v-model="exportFilter.id" placeholder="客户编号" size="mini">
          <template slot="prepend">
            客户编号
          </template>
        </el-input>
        <el-input v-model="exportFilter.phone" placeholder="电话" size="mini">
          <template slot="prepend">
            电话
          </template>
        </el-input>
        <el-input v-model="exportFilter.name" placeholder="姓名" size="mini">
          <template slot="prepend">
            姓名
          </template>
        </el-input>
        <el-input v-model="exportFilter.membershipNumber" placeholder="会员卡号" size="mini">
          <template slot="prepend">
            会员卡号
          </template>
        </el-input>
...
import { getCustomerData, getIntegralsDetails, getLevelData, postClients, getClientsIntegral, putClientsIntegral } from '@/service/PurchaseAndSale/DataEditing/VIPCustomer.js'
import common from '@/mixins/common'
import { BASE_URL } from '@/api/config'
import qs from 'qs'
import { getToken } from '../../../../../../ClientC/src/utils/auth'
...
export default {
  ...
  methods: {
    ...
    confirmHandle2() {
      ...
      this.downloadFile('clients/export', params)
    },
    fileChange: function(el) {
      ...
      if (uploadEventFile == '') {
        ...
        // url为对应的后端接口
        vm.$http.post(BASE_URL + '/main_store/clients/import?storeId=' + this.storeId, formData, config).then(function(res) {
    ...
    changeIntegral(row) {
      ...
      this.$prompt('请输入减少积分', '提示', {
        ...
        data.integral = value
        putClientsIntegral(data).then(res => {
          if (res.data.code !== 1001) {
            this.$message({
              showClose: true,
              message: res.data.message,
