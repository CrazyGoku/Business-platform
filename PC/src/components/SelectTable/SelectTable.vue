    ...
    watch: {
      data: {
        ...
        handler() {
          ...
          let filterArr = ['supplierId','supplierName','remark', 'resultOrderId', 'barCode', 'image', 'goodsLabels', 'origin', 'putaway','prodcingWay', 'pushMoneyStatus', 'discountCouponId']
          let filterArr2 = ['客户电话', '会员卡号']
          for (let i = 0; i < this.dataC.title.length; i++) {
            let column = []
            let v = this.dataC.title[i]
            ...
            this.dataC.items.map(v2 => {
              if (v.key.split('.').length == 2) {
                column.push(v2[v.key.split('.')[0]][v.key.split('.')[1]])
              } else if (v.key.split('.').length == 3) {
                column.push(v2[v.key.split('.')[0]][v.key.split('.')[1]][v.key.split('.')[2]])
              } else {
                column.push(v2[v.key])
              }
            })
            if (!column.every(item => isNaN(Number(item)))) {
              let summary = 0
              column.forEach(v3 => {
                summary += v3
              })
