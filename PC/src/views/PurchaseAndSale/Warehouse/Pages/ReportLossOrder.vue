  ...
  <div>
    ...
    <el-dialog :visible.sync="orderVisible" title="订单详情">
      <el-table :data="orderDetails">
        ...
        />
        <el-table-column
          v-for="(item,index) in ReportLossMap"
          :key="index"
...
import { ReportLossMap, orderType } from '@/views/PurchaseAndSale/Warehouse/config.js'
      ...
      getStorageResultById(params, path).then(res => {
        ...
        if (_data.length > 0) {
          if (!this.isGetSkuMap) {
            ...
            sku.forEach(v => {
              this.ReportLossMap.push({ key: v.key, name: v.key })
            })
