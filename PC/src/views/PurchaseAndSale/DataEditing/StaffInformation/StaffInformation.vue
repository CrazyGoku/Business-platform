<template>
  <div>
    ...
    >
      <div v-for="v1 in functions" :key="v1.id"
           style="margin-bottom: 10px;padding-bottom:5px;border-bottom: 1px solid #e0e0e0">
        <el-checkbox :indeterminate="v1.indeterminate" v-model="v1.checkAll" @change="handleCheckAllChange(v1)">
          全选{{v1.name}}
        </el-checkbox>
        <el-checkbox-group v-model="v1.checkList" @change="handleCheckedChange(v1)">
          <el-checkbox v-for="v2 in v1.children" :key="v2.id" :label="v2.id">{{v2.name}}</el-checkbox>
        </el-checkbox-group>
      </div>
      <span slot="footer" class="dialog-footer">
  ...
  export default {
    ...
    methods: {
      ...
      handleCheckedChange(value) {
        let checkedCount = value.checkList.length;
