    ...
    <el-dialog :close-on-click-modal="false" width="80%" title="发放优惠券" :visible.sync="sendDialog">
      ...
      <div class="dialog-content-input">
        ...
        <el-checkbox-group v-model="checkedLevels" @change="handleCheckedLevelsChange">
          <el-checkbox v-for="(level,index) in levels" :label="level.id" :key="level.id">{{level.name}}</el-checkbox>
        </el-checkbox-group>
        <el-select
          v-model="clientIdS"
          clearable
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button size="mini" type="primary" :disabled="totalSend>=(sendRow.quantity - sendRow.givenQuantity)"
