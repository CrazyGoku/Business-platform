...
(function(win) {
    ...
    var mobile = {
        ...
        postNotification: function(name, params, callback) {
            if (isIOS()) {
                win.hxiOSBridge.handleNotification(name, JSON.stringify(params), callback);
            } else if (isAndroid()) {
                // 生成回调函数方法名称
                ...
                win.hxAndroidBridge.handleNotification(name, JSON.stringify(params), cbName);
            } else {
                console.log("no implementation");
                callback('-999', 'no implementation');
            }
        },
        /**
